...
def list_schedule(dose_date: date | None = None, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    return db.query(ScheduleDose).filter(ScheduleDose.user_id == observed.id, ScheduleDose.dose_date == (dose_date or date.today())).order_by(ScheduleDose.dose_time).all()

@router.post("", response_model=ScheduleOut, status_code=201)
def create_dose(payload: ScheduleCreate, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    observed = get_observed_user(user, db)
    medication = db.query(Medication).filter(Medication.id == payload.medication_id, Medication.user_id == observed.id).first()
    if not medication: raise HTTPException(404, "药品不存在")
    dose = ScheduleDose(user_id=observed.id, **payload.model_dump())
...
def update_dose(dose_id: int, payload: ScheduleUpdate, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    observed = get_observed_user(user, db)
    dose = db.query(ScheduleDose).filter(ScheduleDose.id == dose_id, ScheduleDose.user_id == observed.id).first()
