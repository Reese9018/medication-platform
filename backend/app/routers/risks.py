...
def risks(user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    medications = db.query(Medication).filter(Medication.user_id == observed.id).all()
    return list_risks(observed, medications)
...
def risk_detail(risk_id: str, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    observed = get_observed_user(user, db)
    medications = db.query(Medication).filter(Medication.user_id == observed.id).all()
    all_risks = list_risks(observed, medications)
