...
def list_records(days: int = 7, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    records = db.query(HealthRecord).filter(HealthRecord.user_id == observed.id).order_by(HealthRecord.record_date.desc(), HealthRecord.record_time.desc()).limit(30).all()
...
def create_record(payload: HealthRecordCreate, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    observed = get_observed_user(user, db)
    record = HealthRecord(user_id=observed.id, **payload.model_dump())
...
def summary(user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    observed = get_observed_user(user, db)
    records = db.query(HealthRecord).filter(HealthRecord.user_id == observed.id).order_by(HealthRecord.record_date.desc()).limit(7).all()
