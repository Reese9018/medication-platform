...
def _seed_elder_medications(db: Session, elder: User) -> list[Medication]:
    ...
    meds = [
        Medication(user_id=elder.id, name="硝苯地平缓释片", generic_name="Nifedipine", spec="20mg×14片", dosage="1片", purpose="降压，用于高血压", frequency_per_day=2, times=["08:00", "20:00"], status="active", risk_level="mid", category="降压药", start_date=date.today()-timedelta(days=90), end_date=date.today()+timedelta(days=128), expiry_date=date.today()-timedelta(days=30), remind_before_minutes=30, precautions="避免与葡萄柚汁同服。"),
        Medication(user_id=elder.id, name="二甲双胍缓释片", generic_name="Metformin", spec="500mg×30片", dosage="1片", purpose="降糖，用于2型糖尿病", frequency_per_day=2, times=["08:00", "18:00"], status="active", risk_level="low", category="降糖药", start_date=date.today()-timedelta(days=60), expiry_date=date.today()+timedelta(days=60), remind_before_minutes=30, precautions="餐后服用。"),
        Medication(user_id=elder.id, name="阿司匹林肠溶片", spec="100mg×30片", dosage="1片", purpose="预防心血管事件", frequency_per_day=1, times=["08:00"], status="active", risk_level="mid", category="心血管药", start_date=date.today()-timedelta(days=100), end_date=date.today()+timedelta(days=128), expiry_date=date.today()+timedelta(days=200), remind_before_minutes=30),
        Medication(user_id=elder.id, name="格列美脲片", spec="2mg×30片", dosage="1片", purpose="降糖", frequency_per_day=1, times=["08:00"], status="active", risk_level="high", category="降糖药", start_date=date.today()-timedelta(days=30), end_date=date.today()+timedelta(days=67), expiry_date=date.today()+timedelta(days=45), remind_before_minutes=20, precautions="注意低血糖。"),
        Medication(user_id=elder.id, name="氯氮平片", spec="25mg×100片", dosage="0.5片", purpose="辅助镇静", frequency_per_day=1, times=["22:00"], status="active", risk_level="high", category="精神类药", start_date=date.today()-timedelta(days=120), end_date=date.today()+timedelta(days=21), expiry_date=date.today()-timedelta(days=10), remind_before_minutes=15),
    ]
    db.add_all(meds)
    db.flush()
    return meds
